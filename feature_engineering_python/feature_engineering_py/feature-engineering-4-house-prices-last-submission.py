...
# # It took 26 versions of the code trying absolutely everything I could try from the initial code to move from 600+(14%) to 300-(7%) in the leaderboard, change that can mean a medall in real competions.
# 
# **These are the changes that improve the performnce the most:**
# 1.  **Optuna:** I didnt want to use it because it takes quite a long time and I thought that my parameters were good enough. But I was wrong, Optuna improve a lot the performance of my notebook but... I dont recommend to put it until the end of the code; unless you're okay with spending an hour each time you run the notebook.
# 2. **Drop_uninformative:** a small difference in wich columns were dropped and wich were'nt avoided a lot of overfitting without losing valuable information.
# 3. **Quality-Condition:** both quite informative features that worked really well together.
# 4. **Imputed column:** adding a column pointing what was imputed and what wasn't helped the model to understad better what is real data, what is less reliable data and therefore make better predictions.
