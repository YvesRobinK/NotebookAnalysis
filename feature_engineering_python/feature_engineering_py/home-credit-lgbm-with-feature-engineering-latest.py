...
def application_train_test(num_rows=None, nan_as_category=True):
    ...
    # yüksek korelasyonlu kolonların silinmesi aynı 
...
def bureau_(num_rows=None, nan_as_category=True):
    ...
    # Replacing Extended Credits by 1
...
def kfold_lightgbm(df, num_folds, stratified=False, debug=False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['TARGET'])):
        ...
        feature_importance_df.to_csv("selected_features", index=False)
...
df_ = pd.read_csv("./selected_features")
df_.head()
