...
def build_predict_lgbm(df_train, df_test, gap):
    ...
    for jj in range(100):
        ...
        LGB_PARAMS['seed']  = 3*jj**2+137

        model_cc = lgb.train(LGB_PARAMS, train_set = dtrain_cc, num_boost_round = 300)
        model_ft = lgb.train(LGB_PARAMS, train_set = dtrain_ft, num_boost_round = 300)
    
        # inverse transform from log of change from last known value
        y_pred_cc += (np.expm1(model_cc.predict(df_test, num_boost_round = 300)) + test_lag_cc)/100
        y_pred_ft += (np.expm1(model_ft.predict(df_test, num_boost_round = 300)) + test_lag_ft)/100
...
df_test["ConfirmedCases"] = 0.6 * df_test.ConfirmedCases_test_lgb + 0.4 * df_test.ConfirmedCases_test_mad
df_test["Fatalities"] = 0.6 * df_test.Fatalities_test_lgb + 0.4 * df_test.Fatalities_test_mad
