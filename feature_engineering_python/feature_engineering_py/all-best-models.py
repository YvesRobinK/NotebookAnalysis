...
"""
Python 3.10 Titanic all best models
File name Titanic_all_best_models.py
...
#handmade decision tree - submission score = 0.76315 V22
data_val['Survived'] = mytree(data_val).astype(int)
submit_tree = data_val[['PassengerId','Survived']]
submit_tree.to_csv("sub_tree_0.76315.csv", index=False)

#decision tree w/full dataset modeling submission score: defaults= 0.76555, tuned= 0.77511 V26
submit_dt = tree.DecisionTreeClassifier()
submit_dt = model_selection.GridSearchCV(tree.DecisionTreeClassifier(), param_grid=param_grid, scoring = 'roc_auc', cv = cv_split)
submit_dt.fit(data1[data1_x_bin], data1[Target])
print('Best Parameters: ', submit_dt.best_params_) #Best Parameters:  {'criterion': 'gini', 'max_depth': 4, 'random_state': 0}
data_val['Survived'] = submit_dt.predict(data_val[data1_x_bin])  # 0.77511 V26
submit_decision_tree = data_val[['PassengerId','Survived']]
submit_decision_tree.to_csv("sub_decision_tree_0.77511.csv", index=False)

#bagging w/full dataset modeling submission score: defaults= 0.75119, tuned= 0.78468 V27
submit_bc = ensemble.BaggingClassifier()
submit_bc = model_selection.GridSearchCV(ensemble.BaggingClassifier(), param_grid= {'n_estimators':grid_n_estimator, 'max_samples': grid_ratio, 'oob_score': grid_bool, 'random_state': grid_seed}, scoring = 'roc_auc', cv = cv_split)
submit_bc.fit(data1[data1_x_bin], data1[Target])
print('Best Parameters: ', submit_bc.best_params_) #Best Parameters:  {'max_samples': 0.25, 'n_estimators': 500, 'oob_score': True, 'random_state': 0}
data_val['Survived'] = submit_bc.predict(data_val[data1_x_bin])  # 0.78468 V27
submit_bagging = data_val[['PassengerId','Survived']]
submit_bagging.to_csv("sub_bagging_0.78468.csv", index=False)
submit_bagging.to_csv("submission.csv", index=False)  # best


#extra tree w/full dataset modeling submission score: defaults= 0.76555, tuned= 0.7799 V28
submit_etc = ensemble.ExtraTreesClassifier()
submit_etc = model_selection.GridSearchCV(ensemble.ExtraTreesClassifier(), param_grid={'n_estimators': grid_n_estimator, 'criterion': grid_criterion, 'max_depth': grid_max_depth, 'random_state': grid_seed}, scoring = 'roc_auc', cv = cv_split)
submit_etc.fit(data1[data1_x_bin], data1[Target])
print('Best Parameters: ', submit_etc.best_params_) #Best Parameters:  {'criterion': 'entropy', 'max_depth': 6, 'n_estimators': 100, 'random_state': 0}
data_val['Survived'] = submit_etc.predict(data_val[data1_x_bin])  # 0.7799 V28
submit_extra_tree = data_val[['PassengerId','Survived']]
submit_extra_tree.to_csv("sub_extra_tree_0.7799.csv", index=False)

#random foreset w/full dataset modeling submission score: defaults= 0.71291, tuned= 0.74162 V29
submit_rfc = ensemble.RandomForestClassifier()
submit_rfc = model_selection.GridSearchCV(ensemble.RandomForestClassifier(), param_grid={'n_estimators': grid_n_estimator, 'criterion': grid_criterion, 'max_depth': grid_max_depth, 'random_state': grid_seed}, scoring = 'roc_auc', cv = cv_split)
submit_rfc.fit(data1[data1_x_bin], data1[Target])
print('Best Parameters: ', submit_rfc.best_params_) #Best Parameters:  {'criterion': 'entropy', 'max_depth': 6, 'n_estimators': 100, 'random_state': 0}
data_val['Survived'] = submit_rfc.predict(data_val[data1_x_bin])  # 0.74162 V29
submit_random_foreset = data_val[['PassengerId','Survived']]
submit_random_foreset.to_csv("sub_random_foreset_0.74162.csv", index=False)

#ada boosting w/full dataset modeling submission score: defaults= 0.74162, tuned= 0.75358 V30
submit_abc = ensemble.AdaBoostClassifier()
submit_abc = model_selection.GridSearchCV(ensemble.AdaBoostClassifier(), param_grid={'n_estimators': grid_n_estimator, 'learning_rate': grid_ratio, 'algorithm': ['SAMME', 'SAMME.R'], 'random_state': grid_seed}, scoring = 'roc_auc', cv = cv_split)
submit_abc.fit(data1[data1_x_bin], data1[Target])
print('Best Parameters: ', submit_abc.best_params_) #Best Parameters:  {'algorithm': 'SAMME.R', 'learning_rate': 0.1, 'n_estimators': 300, 'random_state': 0}
data_val['Survived'] = submit_abc.predict(data_val[data1_x_bin])  # 0.75358 V30
submit_ada_boosting = data_val[['PassengerId','Survived']]
submit_ada_boosting.to_csv("sub_ada_boosting_0.75358.csv", index=False)
...
data_val['Survived'] = submit_gbc.predict(data_val[data1_x_bin])  # 0.77272 V31
submit_gradient = data_val[['PassengerId','Survived']]
submit_gradient.to_csv("sub_gradient_0.77272.csv", index=False)
...
data_val['Survived'] = submit_xgb.predict(data_val[data1_x_bin])  # 0.7799 V32
submit_extreme_boosting = data_val[['PassengerId','Survived']]
submit_extreme_boosting.to_csv("sub_extreme_boosting_0.7799.csv", index=False)

#hard voting classifier w/full dataset modeling submission score: defaults= 0.75598, tuned = 0.77751
data_val['Survived'] = vote_hard.predict(data_val[data1_x_bin]) # 0.77751 V25
submit_vote_hard = data_val[['PassengerId','Survived']]
submit_vote_hard.to_csv("sub_vote_hard_0.77751.csv", index=False)

data_val['Survived'] = grid_hard.predict(data_val[data1_x_bin])  # 0.77751 V21
submit_grid_hard = data_val[['PassengerId','Survived']]
submit_grid_hard.to_csv("sub_grid_hard_0.77751.csv", index=False)

#soft voting classifier w/full dataset modeling submission score: defaults= -, tuned = 0.76555
data_val['Survived'] = vote_soft.predict(data_val[data1_x_bin])  # 0.76555 V24
submit_vote_soft = data_val[['PassengerId','Survived']]
submit_vote_soft.to_csv("sub_vote_soft_0.76555.csv", index=False)

data_val['Survived'] = grid_soft.predict(data_val[data1_x_bin])  # 0.75358
submit_grid_soft = data_val[['PassengerId','Survived']]
submit_grid_soft.to_csv("sub_grid_soft_0.75358.csv", index=False)

# The best parameter for AdaBoostClassifier is {'learning_rate': 0.1, 'n_estimators': 300, 'random_state': 0} with a runtime of 51.72 seconds.
# The best parameter for BaggingClassifier is {'max_samples': 0.25, 'n_estimators': 300, 'random_state': 0} with a runtime of 62.83 seconds.
# The best parameter for ExtraTreesClassifier is {'criterion': 'entropy', 'max_depth': 6, 'n_estimators': 100, 'random_state': 0} with a runtime of 95.57 seconds.
# The best parameter for GradientBoostingClassifier is {'learning_rate': 0.05, 'max_depth': 2, 'n_estimators': 300, 'random_state': 0} with a runtime of 57.31 seconds.
# The best parameter for RandomForestClassifier is {'criterion': 'entropy', 'max_depth': 6, 'n_estimators': 100, 'oob_score': True, 'random_state': 0} with a runtime of 162.26 seconds.
# The best parameter for GaussianProcessClassifier is {'max_iter_predict': 10, 'random_state': 0} with a runtime of 9.05 seconds.
# The best parameter for LogisticRegressionCV is {'fit_intercept': True, 'random_state': 0, 'solver': 'newton-cg'} with a runtime of 17.57 seconds.
# The best parameter for BernoulliNB is {'alpha': 0.1} with a runtime of 0.42 seconds.
# The best parameter for GaussianNB is {} with a runtime of 0.08 seconds.
# The best parameter for KNeighborsClassifier is {'algorithm': 'brute', 'n_neighbors': 7, 'weights': 'uniform'} with a runtime of 7.45 seconds.
# The best parameter for SVC is {'C': 2, 'decision_function_shape': 'ovo', 'gamma': 0.1, 'probability': True, 'random_state': 0} with a runtime of 32.69 seconds.
# The best parameter for XGBClassifier is {'learning_rate': 0.01, 'max_depth': 4, 'n_estimators': 300, 'seed': 0} with a runtime of 409.77 seconds.
# Total optimization time was 15.11 minutes.


# <a id="ch12"></a>
# # Step 7: Optimize and Strategize
# ## Conclusion
# Iteration one of the Data Science Framework, seems to converge on 0.77751 submission accuracy. Using the same dataset and different implementation of a decision tree (adaboost, random forest, gradient boost, xgboost, etc.) with tuning does not exceed the 0.77751 submission accuracy. Interesting for this dataset, the simple decision tree algorithm had the best default submission score and with tuning achieved the same best accuracy score.
# 
...
targetName = 'Survived'
idName = 'PassengerId'
competitionDir = '/kaggle/input/titanic'
submission = pd.read_csv('/kaggle/input/titanic/gender_submission.csv')
submission.sort_values(by=[idName], inplace=True)
...
data_val['Survived'] = stats.mode(np.array(preds), axis=0)[0].transpose()
#submit file
submit = data_val[['PassengerId','Survived']]
submit.to_csv("sub_automated_ensemble_0.7799.csv", index=False)  # 0.7799 V36
