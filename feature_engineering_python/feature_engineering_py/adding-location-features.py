...
get_ipython().system('pip install -qq reverse_geocoder')
...
#     n_clusters=20,
#     max_iter=1111,
#     random_state=1337).fit(coordinates)

# cluster_centers = {i: tuple(centroid) for i, centroid in enumerate(tqdm(clustering.cluster_centers_, desc='Creating clusters'))}

# def cluster_features(df):
#     for i, cc in enumerate(cluster_centers.values()):
#         df[f"cluster_{i}"] = df.apply(lambda x: haversine((x['lat'], x['lng']), 
#                                                          cc, unit='ft'), axis=1)
        
#     return df


# raw = cluster_features(raw)
...
#features += ['state_m']
features += ['scale', 'place', 'rot_15_x', 'rot_15_y', 'rot_30_x', 'rot_30_y', 'rot_45_x', 'rot_45_y', 'pca_lat', 'pca_lon', 'umap_lat', 'umap_lon']
...
test1 = test1.fillna(1.7569)  # fill anything you want 
