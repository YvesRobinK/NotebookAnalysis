...
get_ipython().run_cell_magic('time', '', 'train_feat = prepare_df(train)\n')


# In[9]:


get_ipython().run_cell_magic('time', '', 'test_feat = prepare_df(test)\n')


# In[10]:


train_feat.to_parquet('train.parquet')
test_feat.to_parquet('test.parquet')
...
clf.fit(train_feat, train_labels.state)
...
# # to better look at individual features
