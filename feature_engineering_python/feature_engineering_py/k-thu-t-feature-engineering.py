...
# Cài đặt terract
get_ipython().system('{sys.executable} -m pip install tesseract')
...
get_ipython().system('{sys.executable} -m pip install reverse_geocoder')
...
get_ipython().system('{sys.executable} -m pip install user_agents')
...
# Một thông tin quan trọng nữa là đó là IP người dùng có thể dược dùng để phát hiện thông tin quốc gia, tỉnh, huyện, xã,.... Nếu bạn là biết trước điều này bạn sẽ có một cách khôn ngoan hơn để bàn luận chuyện chính trị trên mạng bởi mọi comments của bạn đều bị nhà nước phát hiện địa điểm thông qua IP.
# 
