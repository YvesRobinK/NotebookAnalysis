...
# ![undraw_Font_re_efri (1).png](attachment:99b84759-64cf-4b43-ad7d-fb23af49e22e.png)
# 
# [以前のnotebook](https://www.kaggle.com/code/tsunotsuno/debertav3-lgbm-with-feature-engineering)では、DebertaとLGBMを組み合わせて使用した。
# 
# ところで、Discussionに誤字脱字に関する言及がいくつか見られる。<br>
# 例: https://www.kaggle.com/competitions/commonlit-evaluate-student-summaries/discussion/428941
# 
# 今回はこれに焦点を当てて改善できないか考える。
# 
# In [my previous notebook](https://www.kaggle.com/code/tsunotsuno/debertav3-lgbm-with-feature-engineering), I tried using combination of Deberta and LGBM.
# 
# By the way, I see several references to "misspelling" in Discussion.<br>
# e.g.: https://www.kaggle.com/competitions/commonlit-evaluate-student-summaries/discussion/428941
# 
# In this notebook, I'll try to improve score focus on "misspelling".
# 
# ## Main Concept
# 
# 現在使用しているTransformersモデルのDebertaは「正しい文章」でpretrainされているはずである。
# このモデルに誤字脱字した文章で学習・入力しては、debertaは正しく意味を捉えることができないと予想できる。
# 
# もし私が採点者(人間)ならば、誤字脱字を発見したタイミングで誤字脱字に関する減点を行う。そして、暗黙的に誤字脱字を修正した上で文章中の他の観点での採点を行う。
# 仮に上記のようなプロセスで採点が行われているとすると、誤字脱字をカウントし、**それを修正した上で**debertaに入力することでdebertaは誤字脱字以外の特徴と正しく捉えられるようになるのではないか？
# 
# 今回は、誤字脱字に関して補正した上でdebertaの入力とすることで、誤字脱字とそれ以外をなるべく分離してモデルに評価させることでscoreが向上するかを確認する。
# 
# Currently I use the Deberta model, and this should be pretrained with "correct sentences". If the model is trained with misspelled sentences, I think that Deberta will not be able to capture the meaning correctly.
# 
# If I were a evaluator (human), I would deduct points for misspellings as soon as I discovered them. 
# Then, after implicitly correcting the misspelled words, I would grade the other perspectives in the text. 
# Assuming that the above process is used for scoring, we would expect that counting misspelled words and correcting them before inputting them into deberta would allow deberta to correctly capture the features.
# 
# In this notebook, I'll try to autocorrect misspelling words before input text to the debarta.
# 
# ### Feature Engineering
# 
# 特徴量は前回とほとんど変更せず下記の特徴量を使用する。
# 
# The features are almost unchanged from the previous one.
...
# 
# 図のような構成のモデルを作成する。
# Debertaの入力に使用する`text` について、スペルミスを事前に修正する。
# 他の特徴量作成においては`text`をそのまま使用する。
# 
# Create a model with the configuration shown in the figure.
# I'll correct misspellings in `text` field before input Deberta.
# In other feature engineering, `text` field is used as is.
# 
...
# | Debertav3 + LGBM with spell autocorrect | 0.448 | 0.581 | 0.514 | 0.459 | this notebook |
# 
# 
# CVの値は少しだけ改善されたが、LBの値も良くなった。
# ~~LBはテストデータの13%しか使用していないので、LBが下がったからと言って必ずしも悪いとは言い切れない。
# 少なくともCVは向上しているので、将来的に何かの役に立つかもしれない:)~~
# 
# The CV values improved slightly, and the LB value is improved.
# ~~The LB data is only 13% of all test data, so a lower LB score is not necessarily a bad result.
# Because the CV has improved, this implementation may be useful for something in the future:)~~
# 
