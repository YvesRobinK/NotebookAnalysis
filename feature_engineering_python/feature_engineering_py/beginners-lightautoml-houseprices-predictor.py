...
# It's time to check the RMSE score for the trained model:

# In[19]:


print('RMSE score for the trained model: {:.3f}'.format(
    mean_squared_error(train_data[TARGET_NAME].values, oof_pred.data[:, 0], squared = False)))
...
sub[TARGET_NAME] = test_pred.data[:, 0]
...
sub.to_csv('LightAutoML_submission.csv', index = False)
